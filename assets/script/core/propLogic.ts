...
import { QuadCircleNode } from '../physics/quad_tree';
import { ColliderManager } from '../physics/collider_manager';
const { ccclass, property } = _decorator;


class PropItem extends QuadCircleNode {
    public id: number = -1;
...
export class PropLogic extends Component {
    ...
    private _propItemMap: Map<number, PropItem> = new Map;

    public init (parent: LogicManager) {
        this._parent = parent;

        ColliderManager.instance.bindItemCollider(this.onPorpItemCollider.bind(this));
    }
    ...
    public reset () {
        ...
        keyArray.forEach((element: string) => {
            ...
            ColliderManager.instance.removeItem(this._propItemMap.get(parseInt(element))!);
    ...
    public removeOverProp (length: number) {
        ...
        for (let i = length; i < keyArray.length; i++) {
            if (this._dicProps[i]) {
                ...
                ColliderManager.instance.removeItem(this._propItemMap.get(i));
    ...
    */
    public removePropId (propId: number) {
        ...
        if (prop) {
            ...
            if (this._dicProps[prop.id]) {
                ...
                ColliderManager.instance.removeItem(this._propItemMap.get(prop.id));
    ...
    public generateProp (pos: Vec3, scale: number = 1, type: PropType = PropType.COIN, belongOpenId: string = "", delay: number = 0) {
        ...
        let propItem = new PropItem(this._dicProps[this.indexProp], this.indexProp);
        this._propItemMap.set(this.indexProp, propItem);
        ColliderManager.instance.insertItem(propItem);
     ...
     * @returns 
     */
    public onPorpItemCollider (propItem: PropItem, playerNode: Node, player: Player) {
        let prop = this._parent.currentGameState.props[propItem.id];
        let isAi = player.channel.openId != GobeUtil.instance.ownPlayerId;
        if (prop.type === PropType.HAMMER && player.hammerCount !== 0 || prop.removed) {
            return;
        }
    ...
    public handleProp (player: Player, ndVirtualPlayer: Node, isAi: boolean) {
        ...
        keyPropsArray.forEach((value: string) => {
            ...
            if (geometry.intersect.obbWithOBB(obb1, obb2)) {
                if (prop.type === PropType.HAMMER && player.hammerCount !== 0) {
                    return;
                }
                if (prop.type === PropType.HAMMER) {
                    prop.removed = true;
    ...
    private _canPlaced (pos: Vec3, node: Node) {
        ...
        let rect = { x: pos.x, y: pos.z, radius: node.worldScale.x / 2 };
        return ColliderManager.instance.checkRectIsEmpty(rect);
    }
